...
registerTool({
  ...
  create: (ctx: ToolContext) => ({
    parameters: z.object({
      ...
      number: z.number().nullish().describe("PR number (required for `get`, `comment`, `review`)."),
      title: z.string().nullish().describe("PR title (required for `create`)."),
      body: z.string().nullish().describe("Description or comment (required for `create`, `comment`, `review`). Supports Markdown."),
      head: z.string().nullish().describe("Source branch (required for `create`, e.g. 'feat/my-feature')."),
      base: z.string().nullish().describe("Target branch for `create` (default: main). Filters by base branch in `list`."),
      draft: z.boolean().nullish().describe("If true, create as a draft PR (only for `create`)."),
      includeDiff: z.boolean().nullish().describe("If true, include the diff in the response (only for `get`, truncated at 50KB)."),
      state: z.enum(["open", "closed", "merged", "all"]).nullish().describe("Filter by state (only for `list`, default: open)."),
      author: z.string().nullish().describe("Filter by author (only for `list`)."),
      limit: z.number().nullish().describe("Maximum number of results (only for `list`, default: 100, recommended max: 500)."),
      event: z.enum(["APPROVE", "REQUEST_CHANGES", "COMMENT"]).nullish().describe("Review type (required for `review`)."),
    }),
